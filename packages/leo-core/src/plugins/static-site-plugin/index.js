...
StaticSiteGeneratorWebpackPlugin.prototype.apply = function(compiler) {
  const { locals, renderSrc, outputPaths, options: thisOptions } = this;
  ...
  compiler.plugin("emit", function(compiler, done) {
    ...
    var assetPath = webpackStatsJson.publicPath && thisOptions.fullPath
    ...
    var assetsPluginHash = Object.keys(assetsByChunkName).reduce(function(
      chunkMap,
      chunkName
    ) {
    ...
    try {
      ...
      const webpackAssetsJSON = findAsset(
        "webpack-static-assets.json",
        compiler,
        webpackStatsJson
      );
      ...
      if (typeof render !== "function") {
        throw new Error(
          `Export from "${renderSrc}" must be a function that returns an HTML string`
        );
      }
          ...
          render(
            {
              ...
              assetsPluginHash: assetsPluginHash.bundle ||
                assetsPluginHash.static && assetsPluginHash,
